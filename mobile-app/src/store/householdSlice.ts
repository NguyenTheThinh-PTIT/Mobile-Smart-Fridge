...
import { isGuestUser } from '@/features/account/types';
...
export interface HouseholdMember {
  ...
  email: string;
  name: string | null;
  role: import('@/features/account/types').HouseholdRole;
  joinedAt: string;
  avatar?: string;
}
...
export const selectIsGuestUser = (state: { 
  household: HouseholdState, 
  auth: { user: { id: string } | null } 
}) => {
  const { household, auth } = state;
  const currentHousehold = household.currentHousehold;
  const currentUserId = auth.user?.id ?? null;
  if (!currentHousehold || !currentUserId) return false;
  const currentMember = currentHousehold.members.find((m) => m.id === currentUserId);
  return currentMember ? isGuestUser(currentMember.role) : false;
};
